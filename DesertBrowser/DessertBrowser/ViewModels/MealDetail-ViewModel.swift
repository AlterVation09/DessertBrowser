...
extension MealDetailView {
    @MainActor class ViewModel: ObservableObject {
        ...
        func fetchMealDetails() async {                                         //Ask for the information 'Details' tio the API
            ...
            await fetch(from: detailsURL, onSuccess: handleSuccess)  //On success only if the network request is successful
        }
        ...
        private func setIngredients(using details: MealDetailsResponse) {
            let ingredMeasurements = [       //The array is created by filtering the ingredMeasurements array to remove any elements that have empty names or quantites
